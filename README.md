...
# [다익스트라 알고리즘](./docs/dijkstra.md): 단일 출발점에서 가장 짧은 거리부터 확정해 나간다
...
# [위상 정렬 알고리즘](./docs/topologySort.md): 선후 관계를 지키는 처리 순서를 만든다
* `#dependency`
...
# 수학(Math / Number Theory)
# 에라토스테네스의 체 (Sieve of Eratosthenes): 배수를 지워가며 소수를 빠르게 구한다
* `#math`, `#prime-number`, `#preprocessing`
* 목적/해결 문제: N 이하의 모든 소수 판별 빠른 소수 전처리
* 탐색 방식: 소수의 배수를 반복 제거
* 사용 자료구조: boolean 배열
* 핵심 제약/특징: 
  * i²부터 배수 제거 가능 
