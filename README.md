...
* 탐색 방식: 한 방향으로 끝까지 내려간 후 되돌아오며 탐색한다.
* 자료구조: 스택 기반(재귀 또는 명시적 스택 사용.)
* 방문처리: 무한 루프 방지를 위해 방문 여부 체크 필수이다.
* 구현 방식: 
  * 재귀
  * 반복
* 그래프 유형:
  * 방향 / 무방향 모두 사용 가능
  * 사이클이 있는 경우에도 방문 체크로 처리 가능
* 주요활용
  * 연결요소 탐색
  * 사이클 탐지(방향 / 무방향 그래프)
  * 백트랙킹, 완전탐색
  ...
  * 트리 순회(전위 / 중위 / 후위)
* 탐색 순서 특징:
  * 시작 노드에 따라 결과가 달라질 수 있다.
  * 인접 노드 순서에 따라 탐색 경로가 달라진다.
* 시간 복잡도: O(V + E)
* 공간 복잡도:
  * 재귀 스택 깊이 최대 O(V)
  * 방문 배열 O(V)
...
* 탐색 방식: 시작 노드에서 가까운 노드부터 레벨(거리) 순서로 탐색한다.
* 자료구조: 큐 기반
* 방문 처리: 큐에 넣는 시점에 방문 체크를 수행해야 중복 삽입을 방지한다.
* 그래프 유형
  * 방향 / 무방향 그래프 모두 사용 가능
  * 사이클이 있는 그래프에서도 방문 체크로 처리 가능
* 최단 거리 보장:
  * 모든 간선 가중치가 동일(또는 1)일 때 최단 경로를 보장한다.
* 주요 활용
...
* 탐색 순서 특징:
  * 시작 노드 기준 거리 증가 순서
  * 같은 레벨 내 노드 순서는 큐 삽입 순서에 따라 달라진다.
* 시간 복잡도: O(V + E)
* 공간 복잡도:
  * 큐 최대 크기: O(V)
  * 방문 배열 O(V)
...
# 동적 계획법 (Dynamic programing): 중복 계산을 제거해 최적해 구하기
* `#dp`, `#memoization`, `#optimization`
* 핵심 개념: 큰 문제를 작은 문제로 나누고, 이미 계산한 결과를 저장해 재사용한다.
* 적용 조건: 중복되는 부분 문제, 최적 부분 구조
* 상태 정의: `dp[i]`, `dp[i][j]`처럼 문제의 본질을 표현
* 점화식: 이전 상태로부터 현재 상태를 계산하는 규칙
* 구현 방식:
  * Top-down: 재귀 + 메모이제이션
  * Bottom-up: 반복문 + 테이블
* 주요 활용:
  ...
  * 문자열 문제(LCS, 편집 거리)
  * 배낭 문제
* 탐색 순서 특징: 점화식 의존성을 만족하는 순서로 계산해야 한다.
* 시간 복잡도: 상태 개수 * 전이 비용
* 공간 복잡도: DP 테이블 크기, 상태 압축으로 최적화 가능
